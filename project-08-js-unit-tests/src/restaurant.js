...
function payFunction() {
  let bill = 0;
  let billFoodPrice = 0;
  let billDrinksPrice = 0;
  const billItems = this.consumption;

  for (let i = 0; i < billItems.length; i += 1) {
    if (this.fetchMenu.food[billItems[i]]) {
      billFoodPrice += this.fetchMenu.food[billItems[i]];
    } else if (this.fetchMenu.drinks[billItems[i]]) {
      billDrinksPrice += this.fetchMenu.drinks[billItems[i]];
    }
  }
  bill = billFoodPrice + billDrinksPrice;
  return parseFloat((bill * 1.1).toPrecision(4));
}
